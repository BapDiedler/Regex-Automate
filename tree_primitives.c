...
{
  tree_t fd = copy_tree(right(a));
  change_right(a,left(fd));
  change_left(fd,a);
  a=cons(valueFe(fd),value(fd),left(fd),right(fd));
}
...
tree_t copy_tree(tree_t a){
  if(!is_empty(a)){
    tree_t new_a = new_a = cons(valueFe(a),value(a),copy_tree(left(a)),copy_tree(right(a)));;
    return new_a;
  }
  return cons_empty();
}
...
void change_fe_tree(tree_t a)
{
  int new_fe = 0;
  if(!is_empty(a)){
    new_fe = height(left(a)) - height(right(a));
    change_fe(a,new_fe);
    if(!is_empty(left(a))){
      change_fe_tree(left(a));
    }
    if(!is_empty(right(a))){
      change_fe_tree(right(a));
    }
  }
}
...
// avec un tableau de donnée le remettre dans un arbe
